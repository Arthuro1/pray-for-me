...
| **AI gateway (self-hosted)** | `AI_GATEWAY_URL` (server-only); optional `VITE_AI_GATEWAY_URL` | AI features stay off (they're consent-gated anyway) — no external AI provider is ever used |
...
AI_GATEWAY_URL=http://127.0.0.1:3001             # self-hosted AI gateway (no external provider)
...
- [`ai_rate_limit.sql`](./supabase/ai_rate_limit.sql) — shared per-user per-minute rate limit for AI requests, enforced by the self-hosted gateway (via `api/ai.js`)
- [`20260804120000_encrypted_translations.sql`](./supabase/migrations/20260804120000_encrypted_translations.sql) — encrypted translation cache (keyed-HMAC lookup, AES-GCM ciphertext; no plaintext)
...
React 18 · Tailwind CSS 3 · React Router 7 · Zustand 5 · Vite + vite-plugin-pwa (Workbox) · Supabase (PostgreSQL, RLS, Edge Functions, `pg_cron`) · self-hosted AI gateway (Node/TS → Ollama + Qwen) · idb-keyval (offline queue) · Web Crypto API (E2EE) · web-push (VAPID) · Vitest + GitHub Actions CI · Vercel
