...
export const DAILY_MSG: Record<string, { title: string; generic: string; withCount: string }> = {
  en: { title: 'Time to pray', generic: 'Take a moment with God today.', withCount: 'Prayers for today: {count}' },
  fr: { title: 'Temps de prière', generic: 'Prenez un moment avec Dieu aujourd’hui.', withCount: 'Sujets de prière du jour : {count}' },
  de: { title: 'Zeit zum Beten', generic: 'Nimm dir heute einen Moment mit Gott.', withCount: 'Du hast heute {count} Gebetsanliegen.' },
  pt: { title: 'Hora de orar', generic: 'Reserve um momento com Deus hoje.', withCount: 'Pedidos de oração para hoje: {count}' },
  es: { title: 'Hora de orar', generic: 'Tómate un momento con Dios hoy.', withCount: 'Peticiones de oración para hoy: {count}' },
  ...
  ru: { title: 'Время молитвы', generic: 'Уделите сегодня время Богу.', withCount: 'Молитвенные нужды на сегодня: {count}' },
  ar: { title: 'وقت الصلاة', generic: 'خصص لحظة مع الله اليوم.', withCount: 'طلبات الصلاة لهذا اليوم: {count}' },
  fa: { title: 'زمان دعا', generic: 'امروز لحظه‌ای را با خدا بگذرانید.', withCount: 'امروز {count} موضوع دعا دارید.' },
  hi: { title: 'प्रार्थना का समय', generic: 'आज परमेश्वर के साथ कुछ समय बिताएं।', withCount: 'आज के प्रार्थना विषय: {count}' },
  sw: { title: 'Wakati wa kuomba', generic: 'Tumia muda na Mungu leo.', withCount: 'Mahitaji ya maombi ya leo: {count}' },
  am: { title: 'የጸሎት ጊዜ', generic: 'ዛሬ ከእግዚአብሔር ጋር ጊዜ ይውሰዱ።', withCount: 'የዛሬ የጸሎት ርዕሶች፦ {count}' },
