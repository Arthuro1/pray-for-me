...
const MSG: Record<string, { title: string; body: string; body0: string }> = {
  en: { title: '🙏 Time to pray', body: 'You have {count} prayer subject(s) for today.', body0: 'Take a moment with God today.' },
  fr: { title: '🙏 Temps de prière', body: 'Vous avez {count} sujet(s) de prière aujourd\'hui.', body0: 'Prenez un moment avec Dieu aujourd\'hui.' },
  de: { title: '🙏 Zeit zu beten', body: 'Du hast heute {count} Gebetsanliegen.', body0: 'Nimm dir heute einen Moment mit Gott.' },
  pt: { title: '🙏 Hora de orar', body: 'Você tem {count} pedido(s) de oração hoje.', body0: 'Reserve um momento com Deus hoje.' },
  es: { title: '🙏 Hora de orar', body: 'Tienes {count} petición(es) de oración hoy.', body0: 'Tómate un momento con Dios hoy.' },
  zh: { title: '🙏 祷告时间', body: '今天你有 {count} 个祷告事项。', body0: '今天花点时间与神交通。' },
  ja: { title: '🙏 祈りの時間', body: '今日は {count} 件の祈りの課題があります。', body0: '今日、神様と過ごすひとときを。' },
  ko: { title: '🙏 기도 시간', body: '오늘 {count}개의 기도 제목이 있습니다.', body0: '오늘 하나님과 함께하는 시간을 가지세요.' },
  ru: { title: '🙏 Время молитвы', body: 'У вас сегодня {count} молитвенных просьб.', body0: 'Уделите сегодня время Богу.' },
  ar: { title: '🙏 وقت الصلاة', body: 'لديك {count} طلب(ات) صلاة اليوم.', body0: 'خصص لحظة مع الله اليوم.' },
  fa: { title: '🙏 وقت دعا', body: 'امروز {count} موضوع دعا دارید.', body0: 'امروز لحظه‌ای را با خدا بگذرانید.' },
  hi: { title: '🙏 प्रार्थना का समय', body: 'आज आपके {count} प्रार्थना विषय हैं।', body0: 'आज परमेश्वर के साथ कुछ समय बिताएं।' },
  sw: { title: '🙏 Wakati wa kuomba', body: 'Una maombi {count} leo.', body0: 'Tumia muda na Mungu leo.' },
  am: { title: '🙏 የጸሎት ጊዜ', body: 'ዛሬ {count} የጸሎት ርዕሶች አሉዎት።', body0: 'ዛሬ ከእግዚአብሔር ጋር ጊዜ ይውሰዱ።' },
  id: { title: '🙏 Waktu berdoa', body: 'Anda punya {count} pokok doa hari ini.', body0: 'Luangkan waktu bersama Tuhan hari ini.' },
  tl: { title: '🙏 Oras na manalangin', body: 'May {count} paksa ng panalangin ka ngayon.', body0: 'Maglaan ng sandali kasama ang Diyos ngayon.' },
};
 ...
 try {
  ...
  for (const sub of subs || []) {
    ...
    const [{ data: cats }, { data: prayers }] = await Promise.all([
      ...
      supabase.from('prayers').select('week_days, prayer_categories(category_id)').eq('user_id', sub.user_id).eq('status', 'active'),
    ]);
    const todayCatIds = new Set((cats || []).filter((c: any) => (c.week_days || []).includes(localDow)).map((c: any) => c.id));
    const count = (prayers || []).filter((p: any) => prayerDueToday(p, localDow, todayCatIds)).length;

    const m = MSG[sub.lang] || MSG.en;
    const payload = JSON.stringify({
      title: m.title,
      body: count > 0 ? m.body.replace('{count}', String(count)) : m.body0,
      url: '/',
      tag: 'daily-reminder',
    });
