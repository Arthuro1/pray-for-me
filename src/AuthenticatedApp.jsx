...
function JoinGroupPage() {
  ...
  }, [user?.id, code, joinGroup, lang, navigate]);
...
function AddFriendPage() {
  ...
  useEffect(() => {
    ...
    return () => { cancelled = true; };
  }, [user?.id, id, sendFriendRequestToId, lang, navigate]);
...
export default function AuthenticatedApp({
  initialGuestView = 'landing',
  initialAuthIntent = 'sign-in',
}) {
  ...
  const [guestView, setGuestView] = useState(initialGuestView);
  ...
  useEffect(() => {
    ...
    onMutationDropped((item, err) => {
      const currentLang = usePrayerStore.getState().settings.language || 'fr';
      toast.error(err?.message ? `${item.kind}: ${err.message}` : t(currentLang, 'errorGeneric'));
  ...
  useEffect(() => {
    if (user?.id) {
      ...
      ensureContentLang(usePrayerStore.getState().settings.language || 'fr');
  ...
  }, [user?.id, fetchNotifications, resetNotifications, subscribeNotifications]);
