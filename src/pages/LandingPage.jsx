...
// The honest reassurance printed right under the primary hero CTA: praying needs
// no account, and the guest prayer never leaves the device unless the visitor
// chooses to save it (see FirstPrayerFlow guest mode). Shared map with an English
// fallback — same pattern as BEGIN_LABELS — so every language stays truthful even
// where the fuller landing copy is still an abbreviated placeholder.
const HERO_REASSURANCE = {
  en: 'No account needed. Nothing leaves this device unless you choose to save it.',
  fr: "Aucun compte requis. Rien ne quitte cet appareil, sauf si vous choisissez de l'enregistrer.",
  de: 'Kein Konto nötig. Nichts verlässt dieses Gerät, außer du entscheidest dich, es zu speichern.',
  pt: 'Nenhuma conta necessária. Nada sai deste dispositivo, a menos que você escolha salvar.',
  es: 'Sin cuenta. Nada sale de este dispositivo a menos que elijas guardarlo.',
...
// hero — NOT a Scripture/AI demo (guest mode makes none of those calls). It uses a
// deliberately distinct "Begin a prayer" label (vs. the hero's "Begin with a
// prayer") so the page never promises a verse-finder trial it can't honour, and so
// the two CTAs read as separate invitations. Shared map + English fallback, same
// pattern as BEGIN_LABELS.
const CALLOUT_BEGIN_LABELS = {
  en: 'Begin a prayer',
  fr: 'Commencez une prière',
  de: 'Ein Gebet beginnen',
  pt: 'Comece uma oração',
  es: 'Comienza una oración',
  zh: '开始一个祷告',
...
const CONTENT = {
  fr: {
    ...
    faqs: [
      ...
      { q: 'Ai-je besoin d\'un compte ?', a: 'Non — vous pouvez prier une première prière sans compte, et elle reste sur votre appareil. Un compte gratuit ne sert qu\'à enregistrer vos prières et à les synchroniser entre vos appareils ; inscrivez-vous avec Google en un tap ou par email/mot de passe.' },
  ...
  en: {
    ...
    features: [
      ...
      { icon: Lock, color: '#6d28d9', title: 'Private & transparent', desc: 'Your prayers are stored with row-level security — only you can see them. Private prayers are end-to-end encrypted by default, from the moment you sign in — no setup, no passphrase to remember. Add an optional passphrase and one-time recovery code to unlock a new device. AI features send only the prayer title; we are transparent about what is shared.' },
    ],
    ...
    ctaSub: 'No account needed to begin — your prayer stays on this device until you choose to save it. Free, private, and available in 16 languages.',
  ...
  de: {
    ...
    features: [
      ...
      { icon: Lock, color: '#6d28d9', title: 'Privat & sicher', desc: 'Deine Gebete verlassen nie deinen Account — nur du kannst sie sehen. Private Gebete sind standardmäßig Ende-zu-Ende-verschlüsselt, ab dem Moment der Anmeldung — ohne Einrichtung, ohne Passphrase zum Merken. Füge optional eine Passphrase und einen einmaligen Wiederherstellungscode hinzu, um ein neues Gerät zu entsperren.' },
    ],
    ...
    faqs: [
      ...
      { q: 'Brauche ich einen Account?', a: 'Nein — du kannst ein erstes Gebet ganz ohne Account beten, und es bleibt auf deinem Gerät. Ein kostenloser Account ist nur nötig, um Gebete zu speichern und über deine Geräte zu synchronisieren; melde dich mit Google in einem Tap an oder nutze E-Mail/Passwort.' },
    ...
    ctaSub: 'Kein Account zum Beginnen nötig — dein Gebet bleibt auf diesem Gerät, bis du dich entscheidest, es zu speichern. Kostenlos, privat und in 16 Sprachen verfügbar.',
  ...
  pt: {
    ...
    features: [
      ...
      { icon: Lock, color: '#6d28d9', title: 'Privado & seguro', desc: 'Suas orações nunca saem da sua conta — somente você pode vê-las. As orações privadas são criptografadas de ponta a ponta por padrão, desde o momento em que você entra — sem configuração, sem frase secreta para lembrar. Adicione uma frase secreta e um código de recuperação opcionais para desbloquear um novo dispositivo.' },
    ],
    ...
    faqs: [
      ...
      { q: 'Preciso de uma conta?', a: 'Não — você pode fazer uma primeira oração sem conta, e ela fica no seu dispositivo. Uma conta gratuita só é necessária para salvar orações e sincronizá-las entre seus dispositivos; cadastre-se com Google em um toque ou use email/senha.' },
    ...
    ctaSub: 'Nenhuma conta necessária para começar — sua oração fica neste dispositivo até você escolher salvá-la. Gratuito, privado e disponível em 16 idiomas.',
  ...
  ja: {
    ...
    features: [
      ...
      { icon: Lock, color: '#6d28d9', title: 'プライベート＆安全', desc: 'あなたの祈りはアカウントの外に出ません。プライベートな祈りは、ログインした瞬間から既定でエンドツーエンド暗号化されます — 設定も、覚えるパスフレーズも不要です。新しい端末のロックを解除するために、任意でパスフレーズと復旧コードを追加できます。' },
    ],
    stepsTitle: '使い方',
    ...
    faqs: [
      ...
      { q: 'アカウントが必要ですか？', a: 'いいえ — 最初の祈りはアカウントなしで祈れて、その祈りは端末内にとどまります。無料アカウントは、祈りを保存して複数のデバイスで同期するときにだけ必要です。Googleでワンタップ登録するか、メール/パスワードを使用してください。' },
  ...
  sw: {
    ...
    features: [
      ...
      { icon: Lock, color: '#6d28d9', title: 'Ya siri na salama', desc: 'Maombi yako hayatoki kamwe katika akaunti yako. Maombi ya faragha husimbwa kutoka mwanzo hadi mwisho kwa chaguo-msingi, tangu unapoingia — bila usanidi wala neno la siri la kukumbuka. Ongeza neno la siri na msimbo wa kurejesha wa hiari ili kufungua kifaa kipya.' },
    ],
    ...
    faqs: [
      ...
      { q: 'Je, ninahitaji akaunti?', a: 'Hapana — unaweza kuomba ombi la kwanza bila akaunti, nalo linabaki kwenye kifaa chako. Akaunti ya bure inahitajika tu kuhifadhi maombi na kuyasawazisha kwenye vifaa vyako; jisajili na Google kwa kugonga mara moja au tumia barua pepe/neno la siri.' },
  ...
  am: {
    ...
    features: [
      ...
      { icon: Lock, color: '#6d28d9', title: 'የግል እና ደህንነቱ የተጠበቀ', desc: 'ጸሎቶችዎ ከመለያዎ ፈጽሞ አይወጡም። የግል ጸሎቶች ከገቡበት ቅጽበት ጀምሮ በነባሪነት ከጫፍ እስከ ጫፍ ይመሰጠራሉ — ማዋቀር የለም፣ የሚያስታውሱት የሚስጥር ሐረግ የለም። አዲስ መሣሪያ ለመክፈት በአማራጭ የሚስጥር ሐረግ እና የማገገሚያ ኮድ ያክሉ።' },
    ],
    ...
    faqs: [
      ...
      { q: 'መለያ ያስፈልጋል?', a: 'አይ — የመጀመሪያ ጸሎትን ያለ መለያ መጸለይ ይችላሉ፣ እርሱም በመሣሪያዎ ላይ ይቆያል። ነፃ መለያ የሚያስፈልገው ጸሎቶችን ለማስቀመጥ እና በመሣሪያዎችዎ መካከል ለማስተባበር ብቻ ነው፤ በ Google አንድ ጫን ወይም ኢሜይል/የምስጢር ቃል ይጠቀሙ።' },
  ...
  id: {
    ...
    features: [
      ...
      { icon: Lock, color: '#6d28d9', title: 'Pribadi & aman', desc: 'Doa Anda tidak pernah keluar dari akun Anda. Doa pribadi dienkripsi ujung-ke-ujung secara bawaan, sejak Anda masuk — tanpa penyiapan, tanpa frasa sandi untuk diingat. Tambahkan frasa sandi dan kode pemulihan opsional untuk membuka perangkat baru.' },
    ],
    ...
    faqs: [
      ...
      { q: 'Apakah perlu akun?', a: 'Tidak — Anda dapat memanjatkan doa pertama tanpa akun, dan doa itu tetap di perangkat Anda. Akun gratis hanya diperlukan untuk menyimpan doa dan menyinkronkannya di seluruh perangkat Anda; daftar dengan Google dalam satu ketukan.' },
      { q: 'Apakah gratis?', a: 'Ya, sepenuhnya gratis. Aplikasi ini adalah open source.' },
    ],
    ctaTitle: 'Mulai jurnal doa Anda hari ini',
    ctaSub: 'Tidak perlu akun untuk memulai — doa Anda tetap di perangkat ini sampai Anda memilih untuk menyimpannya. Gratis, pribadi, dan tersedia dalam 16 bahasa.',
  ...
  tl: {
    ...
    features: [
      ...
      { icon: Lock, color: '#6d28d9', title: 'Pribado at ligtas', desc: 'Ang iyong mga panalangin ay hindi lumalabas sa iyong account. Ang mga pribadong panalangin ay end-to-end encrypted bilang default, mula sa sandaling mag-sign in ka — walang setup, walang passphrase na kabisaduhin. Magdagdag ng opsyonal na passphrase at recovery code para i-unlock ang bagong device.' },
    ],
    ...
    faqs: [
      ...
      { q: 'Kailangan ko ba ng account?', a: 'Hindi — maaari kang manalangin ng unang panalangin nang walang account, at mananatili ito sa iyong device. Kailangan lang ang libreng account para i-save ang mga panalangin at i-sync ang mga ito sa iyong mga device; mag-sign up gamit ang Google sa isang tap.' },
      { q: 'Libre ba?', a: 'Oo, ganap na libre. Ang app na ito ay open source.' },
    ],
    ctaTitle: 'Simulan ang iyong talaarawan ng panalangin ngayon',
    ctaSub: 'Hindi kailangan ng account para magsimula — mananatili ang iyong panalangin sa device na ito hanggang piliin mong i-save ito. Libre, pribado, at makukuha sa 16 na wika.',
  ...
  ko: {
    ...
    features: [
      ...
      { icon: Lock, color: '#6d28d9', title: '비공개 및 안전', desc: '당신의 기도는 계정 밖으로 나가지 않습니다. 비공개 기도는 로그인하는 순간부터 기본적으로 종단 간 암호화됩니다 — 설정도, 기억할 암호문구도 필요 없습니다. 새 기기의 잠금을 해제하려면 선택적으로 암호문구와 복구 코드를 추가하세요.' },
    ],
    stepsTitle: '작동 방법',
    ...
    faqs: [
      ...
      { q: '계정이 필요한가요?', a: '아니요 — 계정 없이도 첫 기도를 드릴 수 있으며, 그 기도는 당신의 기기에 남습니다. 무료 계정은 기도를 저장하고 여러 기기에서 동기화할 때만 필요합니다. Google로 한 번 탭하여 가입하거나 이메일/비밀번호를 사용하세요.' },
  ...
  ru: {
    ...
    features: [
      ...
      { icon: Lock, color: '#6d28d9', title: 'Приватно и безопасно', desc: 'Ваши молитвы никогда не покидают ваш аккаунт. Личные молитвы шифруются сквозным шифрованием по умолчанию — с момента входа, без настройки и без секретной фразы, которую нужно запоминать. Добавьте необязательную секретную фразу и код восстановления, чтобы разблокировать новое устройство.' },
    ],
    ...
    faqs: [
      ...
      { q: 'Нужен ли аккаунт?', a: 'Нет — вы можете помолиться первой молитвой без аккаунта, и она останется на вашем устройстве. Бесплатный аккаунт нужен только для того, чтобы сохранять молитвы и синхронизировать их между устройствами; зарегистрируйтесь через Google одним нажатием.' },
  ...
  ar: {
    ...
    features: [
      ...
      { icon: Lock, color: '#6d28d9', title: 'خاص وآمن', desc: 'صلواتك لا تغادر حسابك أبداً. الصلوات الخاصة مشفّرة من طرف إلى طرف افتراضياً، منذ لحظة تسجيل دخولك — بلا إعداد وبلا عبارة مرور تحفظها. أضِف عبارة مرور ورمز استرداد اختياريين لفتح جهاز جديد.' },
    ],
    stepsTitle: 'كيف يعمل',
    ...
    faqs: [
      ...
      { q: 'هل أحتاج حساباً؟', a: 'لا — يمكنك أن تصلّي صلاة أولى دون حساب، وتبقى على جهازك. الحساب المجاني مطلوب فقط لحفظ الصلوات ومزامنتها عبر أجهزتك؛ سجّل بـ Google بنقرة واحدة.' },
  ...
  fa: {
    ...
    features: [
      ...
      { icon: Lock, color: '#6d28d9', title: 'خصوصی و امن', desc: 'دعاهای شما هرگز از حساب شما خارج نمی‌شوند. دعاهای خصوصی به‌طور پیش‌فرض و از لحظهٔ ورود، سرتاسری رمزگذاری می‌شوند — بدون پیکربندی و بدون عبارت عبوری که باید به خاطر بسپارید. برای باز کردن دستگاه جدید، به‌صورت اختیاری یک عبارت عبور و کد بازیابی اضافه کنید.' },
    ],
    ...
    faqs: [
      ...
      { q: 'آیا به حساب نیاز دارم؟', a: 'خیر — می‌توانید بدون حساب اولین دعا را انجام دهید و روی دستگاه شما می‌ماند. حساب رایگان فقط برای ذخیرهٔ دعاها و همگام‌سازی آن‌ها میان دستگاه‌هایتان لازم است؛ با یک ضربه با Google ثبت‌نام کنید.' },
  ...
  zh: {
    ...
    features: [
      ...
      { icon: Lock, color: '#6d28d9', title: '私密且安全', desc: '您的祷告永远不会离开您的账户。私密祷告自您登录起便默认端到端加密——无需设置，也无需记住任何密码短语。可选择添加密码短语和恢复码，用于解锁新设备。' },
    ],
    stepsTitle: '使用方法',
    ...
    faqs: [
      ...
      { q: '我需要账户吗？', a: '不需要——你可以在没有账户的情况下祷告第一个祷告，它会保留在你的设备上。免费账户只在你想保存祷告并在各设备间同步时才需要；一键用Google注册，或使用邮箱/密码。' },
  ...
  es: {
    ...
    features: [
      ...
      { icon: Lock, color: '#6d28d9', title: 'Privado y seguro', desc: 'Tus oraciones nunca salen de tu cuenta — solo tú puedes verlas. Las oraciones privadas se cifran de extremo a extremo de forma predeterminada, desde el momento en que inicias sesión — sin configuración ni frase de contraseña que recordar. Añade una frase de contraseña y un código de recuperación opcionales para desbloquear un dispositivo nuevo.' },
    ],
    ...
    faqs: [
      ...
      { q: '¿Necesito una cuenta?', a: 'No — puedes orar una primera oración sin cuenta, y se queda en tu dispositivo. Una cuenta gratuita solo hace falta para guardar oraciones y sincronizarlas entre tus dispositivos; regístrate con Google en un toque o usa email/contraseña.' },
    ...
    ctaSub: 'No necesitas cuenta para empezar — tu oración se queda en este dispositivo hasta que elijas guardarla. Gratis, privado y disponible en 16 idiomas.',
  ...
  hi: {
    ...
    features: [
      ...
      { icon: Lock, color: '#6d28d9', title: 'निजी और सुरक्षित', desc: 'आपकी प्रार्थनाएँ कभी आपके खाते से बाहर नहीं जातीं। निजी प्रार्थनाएँ साइन इन करते ही डिफ़ॉल्ट रूप से एंड-टू-एंड एन्क्रिप्टेड होती हैं — कोई सेटअप नहीं, याद रखने के लिए कोई पासफ़्रेज़ नहीं। नए डिवाइस को अनलॉक करने के लिए वैकल्पिक रूप से एक पासफ़्रेज़ और रिकवरी कोड जोड़ें।' },
    ],
    ...
    faqs: [
      ...
      { q: 'क्या मुझे खाते की जरूरत है?', a: 'नहीं — आप बिना खाते के अपनी पहली प्रार्थना कर सकते हैं, और वह आपके डिवाइस पर ही रहती है। मुफ्त खाता केवल प्रार्थनाओं को सहेजने और उन्हें अपने सभी डिवाइस पर सिंक करने के लिए चाहिए; एक टैप में Google से साइन अप करें या ईमेल/पासवर्ड का उपयोग करें।' },
